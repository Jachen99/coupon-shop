package space.jachen.userapp.mapper;

import space.jachen.userapp.domain.TUser;
import space.jachen.userapp.domain.TUserExample;
...
public class TUserSqlProvider {
    ...
    protected void applyWhere(TUserExample example, boolean includeExamplePhrase) {
        ...
        List<TUserExample.Criteria> oredCriteria = example.getOredCriteria();
        boolean firstCriteria = true;
        for (int i = 0; i < oredCriteria.size(); i++) {
            TUserExample.Criteria criteria = oredCriteria.get(i);
            if (criteria.isValid()) {
                if (firstCriteria) {
                    firstCriteria = false;
                } else {
                ...
                sb.append('(');
                List<TUserExample.Criterion> criterions = criteria.getAllCriteria();
                boolean firstCriterion = true;
                for (int j = 0; j < criterions.size(); j++) {
                    TUserExample.Criterion criterion = criterions.get(j);
                    if (firstCriterion) {
